...
Algoritma LOS Guidance menghitung:
1. Cross-track error (jarak ke path)
2. Yaw reference berdasarkan LOS angle
3. Velocity reference (Vx, Vy, Vz) berdasarkan vektor LOS

Fitur:
- LOS Guidance untuk path following (2D dan 3D)
- Lookahead distance adaptif berdasarkan kecepatan
- Cross-track error integration untuk mengurangi steady-state error
- Support multiple trajectory types: circle, square, helix, diamond
- Altitude gate untuk takeoff safety

Published Topics:
    /trajectory/ref_pose   (PoseStamped, ENU) - Position reference
    /trajectory/ref_vel    (TwistStamped, ENU) - Velocity reference dari LOS
    /trajectory/los_info   (TwistStamped)      - LOS debug info (cross-track, along-track, etc.)
...
Author: Generated for MPC path following
"""
...
class LOSGuidanceTrajectoryPublisher(object):
    """
    Line of Sight (LOS) Guidance untuk path following.
    ...
    6. Velocity reference: arah ke lookahead dengan magnitude ref_speed
    """
    ...
    def compute_los_guidance(self, pos_ned):
        ...
        Returns:
            ...
            vel_ref_ned: Velocity reference (NED)
