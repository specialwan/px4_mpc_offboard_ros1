...
# -*- coding: utf-8 -*-
"""
Data Logger Node for MPC/MAVROS Trajectory Tracking (ROS1 version)
...
- Menggunakan topik MAVROS + trajectory:
  ...
  - /trajectory/ref_pose          (🔁 trajectory reference position)
...
class DataLoggerMavrosTraj(object):
    def __init__(self):
        ...
        base = f'flight_log_traj_{timestamp}'
        ...
        self.controller_type = 'MPC_TRAJECTORY'  # beda dengan logger lama
        ...
        # 🔁 Trajectory reference (yang di-follow MPC)
        ...
        # info awal
        rospy.loginfo('=' * 70)
        rospy.loginfo('📊 DATA LOGGER MAVROS TRAJECTORY (ROS1) STARTED')
        rospy.loginfo('=' * 70)
    ...
    def trajectory_pose_callback(self, msg: PoseStamped):
        """
        Trajectory reference pose (ENU) → disimpan sebagai target_pos + target_yaw.
        Inilah lintasan referensi yang akan dipakai untuk menghitung trajectory error.
        """
        if self._shutting_down:
            return
        ...
        # untuk flag "fresh point" (dulu waypoint_fresh, sekarang ref_fresh)
        self.last_ref_time = time.monotonic()

        if not self.traj_ref_received:
            self.traj_ref_received = True
            rospy.loginfo('✅ Trajectory reference pose received (/trajectory/ref_pose)')
    ...
    def on_shutdown(self):
        ...
        rospy.loginfo('🔻 ROS shutdown: saving final CSV/Excel (trajectory logger)...')
        try:
            self.save_to_excel()
        except Exception:
            pass
...
def main():
    rospy.init_node('data_logger_mavros_traj', anonymous=False)
    node = DataLoggerMavrosTraj()
    try:
        rospy.spin()
